defmodule Ctf.Game do
  ...
  def start_battle(health_points: health_points) do
    ...
    played_games =
      combos
      |> Enum.map(fn contenders ->
        Task.async(fn ->
          {contenders, do_game(contenders: contenders, health_points: health_points)}
        end)
      end)
  ...
  defp perform_game_loop(:ok, game, frames, count) do
    ...
    game_with_updated_player_states = %__MODULE__{
      game
      | board: %Board{game.board | players: players}
    }
        ...
        fn {p1_move, p2_move}, {status, [latest_frame | _] = acc} ->
          if status == :ok do
            ...
            move_players_maybe_reversed =
              cond do
                !is_nil(p1_move) && !is_nil(p2_move) && elem(p1_move, 0) != :fire &&
                    elem(p2_move, 0) == :fire ->
            ...
            {frame_status, newest_frames} =
              create_frame(:ok, move_players_with_numbers_only, [frame_with_cleared_events])
  ...
  defp create_frame(status, [{{:fire, count}, player_number} | rest_steps], frames) do
    ...
    player =
      %Player{x: x, y: y, number: ^player_number} =
  ...
  defp create_frame(status, [{{direction, 1}, player_number} | rest_steps], frames)
       when direction in [:clockwise, :counterclockwise] do
    [newest_frame | rest_frames] = frames

    player =
      %Player{number: ^player_number} = Enum.at(newest_frame.board.players, player_number - 1)
    ...
    updated_frame_with_event = %__MODULE__{
      updated_frame
      | board: %Board{updated_frame.board | events: [rotation_event]}
    }

    updated_frames =
      cond do
        length(newest_frame.board.events) > 0 &&
            elem(Enum.at(newest_frame.board.events, 0), 0) == :fire ->
        ...
        true ->
          updated_frame_with_prepended_events = %__MODULE__{
            updated_frame
            | board: %Board{
                updated_frame.board
  ...
  defp create_frame(status, [{{:move, 1}, player_number} | rest_steps], frames) do
    ...
    player =
      %Player{x: x, y: y, health_points: health_points, number: ^player_number} =
      Enum.at(newest_frame.board.players, player_number - 1)
    ...
    case detect_collision(player, newest_frame, {x, y}, 1) do
      ...
      {type, _} = barrier when type in [:edge, :obstacle] ->
        ...
        case {new_player.health_points, status} do
          {0, :ok} ->
            [other_player] =
              Enum.filter(updated_frame.board.players, fn prospect -> prospect != new_player end)
  ...
  defp no_move_and_reduce_health(
         newest_frame,
         rest_frames,
         status,
         rest_steps,
    ...
    case status do
      :ok ->
        ...
        # reduce both players
        new_players =
          Enum.map(newest_frame.board.players, fn %Player{health_points: health_points} =
                                                    board_player ->
  ...
  defp lockstep(move_lists, count) do
    [p1_moves, p2_moves] = Enum.map(move_lists, fn moves -> first(moves, [], count) end)
